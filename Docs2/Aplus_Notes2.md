...
2. `Tasklist` cmd opens CLI v of task manager
...
5. Registry root keys are org. into sub keys & values
...
11. Task Scheduler (Windows management utility) allows config an app to run auto or at regular intervals
...
9. Encrypting File System (EFS) enables you to encrypt files, rendering them invisible by other accts.

### OS maintenance, optimization, general info
...
- **Corp vs Personal needs**
  ...
  - BitLocker: drive encryption utility. data on disk is encrypted to prevent unauthorized access (useful for laptops that leave office env & can be lost/stolen)
  ...
  - EFS: Encrypting File System; fs feat. that's config. to encrypt vol, dir, fi
    - protects data from physical attacker
...
- GPS nav system that uses satellites to get pinpoint position has 3 primary components
...
- Connectors are installed at terminating point of cable and allow components & peripherals to connect
  - The type used depends on the cable and what receptacle it needs connected to.
...
3. `F-typ`e: F connector; used w/ coaxial cables for cable/satellite data connections
